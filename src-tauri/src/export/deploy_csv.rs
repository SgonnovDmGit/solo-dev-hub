...
//! CSV-formats them (RFC4180), the command layer writes the file. Keeping the
//! formatter here (rather than inline in the command) makes escaping
//! cargo-testable.
...
/// RFC4180 field escaping: if the field contains a comma, a double-quote, a
/// carriage return, or a newline, wrap it in double quotes and escape embedded
/// `"` as `""`. Fields without those characters are emitted raw.
...
/// Format the deploy-report rows as an RFC4180 CSV string.
///
...
mod tests {
    ...
    #[test]
    fn test_plain_row_emitted_raw_comma_separated() {
