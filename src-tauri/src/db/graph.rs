...
impl AppDb {
    ...
    pub fn get_project_graph(&self, project_id: i64) -> SqlResult<ProjectGraph> {
        ...
        let (center, ring_repos): (Repository, Vec<Repository>) = if project_type == "standard" {
            ...
            let center_idx = repos
                .iter()
                .position(|r| r.role.as_deref() == Some("server"))
                .unwrap_or(0);
        ...
        let mut edges: Vec<GraphEdge> = ring
            .iter()
            .map(|n| GraphEdge {
...
mod tests {
    ...
    fn test_get_project_graph_server_project() {
        ...
        let server = db
            .insert_local_repository("/s", "server-repo", Some(p.id), Some("server"))
            .unwrap();
        let _client = db
            .insert_local_repository("/c", "client-repo", Some(p.id), Some("client"))
            .unwrap();
        ...
        let has_inproj = g
            .edges
            .iter()
            .any(|e| matches!(e.kind, GraphEdgeKind::InProject));
        let has_cross = g
            .edges
            .iter()
            .any(|e| matches!(e.kind, GraphEdgeKind::CrossProjectMs));
        assert!(has_inproj);
        assert!(has_cross);
    }
    ...
    fn test_get_project_graph_microservice_project_returns_parent_servers() {
        let db = make_db();
        let ms = db.create_project("MS", None, "microservice").unwrap();
        db.insert_local_repository("/ms", "ms-repo", Some(ms.id), Some("server"))
            .unwrap();
        ...
        assert!(
            has_parent,
            "microservice graph should list parent server projects"
        );
    }
    ...
    fn test_get_project_graph_no_server_role_uses_first_repo() {
        ...
        let r1 = db
            .insert_local_repository("/a", "a-repo", Some(p.id), Some("client"))
            .unwrap();
        db.insert_local_repository("/b", "b-repo", Some(p.id), Some("tool"))
            .unwrap();
        let g = db.get_project_graph(p.id).unwrap();
        let center = g.center.expect("center exists");
        assert_eq!(
            center.repo_id,
            Some(r1.id),
            "fallback to first repo when no server"
        );
    }
}
