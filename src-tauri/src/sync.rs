...
pub fn generate_project_md(db: &AppDb, project_id: i64, repo_root: &Path) -> Result<(), String> {
    ...
    md.push_str("---\n_Auto-generated by GitHub Repo Manager. Do not edit manually — changes will be overwritten on next sync._\n");
    ...
    let (name, type_display, description, repos_table, ms_block, parents_block) =
        if let Some(pid) = project_id {
            ...
            (
                "—".to_string(),
                "—".to_string(),
                "Global AI instructions (configured by the user in GitHub Repo Manager). For per-project context see CLAUDE.md of the specific repo.".to_string(),
