...
- **Deploy report CSV export (T-000140).** A 📥 "Export CSV" button on the deploy report writes the currently-filtered rows (WYSIWYG: repo, environment, domain, branch, image tag, DB name, secrets count, updated) to a user-chosen path via the OS save dialog. Semicolon-delimited (so RU/EU-locale Excel opens it into columns directly) with RFC4180 quoting, stable English headers, no new dependency; sensitive values stay masked exactly as in the UI. The frontend flattens the displayed rows and the backend only formats + writes (single-source display logic, cargo-tested escaping).
