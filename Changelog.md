...
### Tests
- 375 cargo (+5 new for `sync::confirm_pair`: client→server, server→MS happy paths, sibling-NNN disambig invariant from v0.27.1 preserved, unknown source role errors out, missing local_path silent no-op) / 72 vitest / 0 svelte issues on 495 files.
