...
- **Отчёт по деплоям** — портфельный обзор всех deploy-окружений по всем репо в одном экране (домены, ветки, image-tag'и, число секретов), сгруппированный по проектам. Клик по строке → переход к настройке этого окружения.
- **Наборы секретов** — переиспользуемые, локально-зашифрованные наборы значений секретов (SSH / DB / npm ключи, общие для нескольких серверов). Вводишь один раз — применяешь к секретам любого репо или deploy-окружения, вместо повторного ввода в каждом репо. AES-256-GCM at-rest, ключ в OS keyring — без мастер-пароля.
...
- **Шаблоны** — per-language seed'ы для `.gitignore`, `.gitattributes`, deploy YAML, CLAUDE.md секций. Настраиваешь один раз в приложении — синхронизирует во все проекты.
...
- **Один .exe, ~19 MB** — Tauri v2 + WebView2. Без Electron-bloat'а. Без daemon'а. Без телеметрии. Единственный фоновый сетевой вызов — update-checker ходит в GitHub Releases один раз при старте; всё остальное только по твоему явному действию.
...
- **i18n** — Русский (по умолчанию) + Английский, ~850 type-safe ключей, без runtime-зависимостей
...
Начни с [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) — карта слоёв, несущие архитектурные решения с обоснованием и кросс-репо flow требований. Схема БД, ER-диаграмма и по-табличные заметки — в [docs/schema/](docs/schema/); разборы отдельных фич — в [docs/flows/](docs/flows/).
...
- **v1.10.0** *(текущий — 2026-07-19)* — документация для контрибьюторов: [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md) и генерируемый справочник по схеме SQLite ([`docs/schema/`](docs/schema/), tbls — ER-диаграмма плюс страница на каждую таблицу).
- **v1.9.x** — набор AI-правил переехал из always-on инструкций в 12 ролевых on-demand скиллов; авто-коммит уважает `.gitignore`; синк уехал с UI-потока; закрытие кросс-репо требований укреплено.
- **v1.8.0** — раздел «Отчёты»: портфельный отчёт по деплоям и аудит отправки секретов, с CSV-экспортом.
- **v1.7.0** — автоматизация воркфлоу: авто-синк по таймеру, авто-коммит синканных кросс-репо файлов, авто-закрытие требований через `.impl.md`.
- **v1.6.0** — значения деплой-секретов переживают перезапуск, зашифрованные at-rest.
- **v1.5.0** — UI-сплиты (детали проекта и панель секретов).
- **v1.4.x** — веха внутреннего рефакторинга: слой Rust-команд и TS-биндинги разложены по доменным модулям.
- **v1.3.0** — переиспользуемые зашифрованные наборы секретов: вводишь SSH / DB / npm значения один раз, применяешь к любому репо или deploy-окружению.
- **v1.2.0** — портфельный отчёт по деплоям с drill-down; managed-шаблон `.gitattributes`.
- **v1.1.0** — verdict-rollback для багов (↩ reopen на confirmed/rejected).
- **v1.0.0** — public launch (2026-05-18), MIT open source.
