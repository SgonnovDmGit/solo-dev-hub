...
- **i18n** — Русский (по умолчанию) + Английский, ~790 type-safe ключей, без runtime-зависимостей
...
```bash
cd src-tauri && cargo test --lib   # ~400 Rust-тестов
npm test                            # vitest на фронтенде (~85 тестов)
...
- **v1.4.1** *(текущий — 2026-06-15)* — patch: декомпозировали обработчик `sync_project` из слоя команд в доменный модуль `sync/project_sync.rs` (без изменения поведения), плюс dev-self-heal, авто-освобождающий порт 1420 перед `tauri dev`.
- **v1.4.0** — веха внутреннего рефакторинга: разбили слой Rust-команд (`lib.rs` → `commands/`) и TS-биндинги (`tauri-commands.ts` → каталог), вынесли ресайзер сайдбара в отдельный компонент; заодно полиш экрана наборов (читаемое multiline-редактирование `SSH_KEY`) и фикс сломанной темы (2026-06-14).
