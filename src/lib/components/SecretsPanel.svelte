<script lang="ts">
  ...
  const selectedWithValues = $derived(
    [...selectedSecrets].filter(name => secretValues[name]?.trim())
  );
  ...
  async function handleUpdateSelected() {
    ...
    const toUpdate = selectedWithValues;
    if (toUpdate.length === 0) return;

    pushing = true;
    try {
      const { owner, repo } = splitRepoFullName(repoFullName);
      const { key, key_id } = await getRepoPublicKey($pat, owner, repo);
      const repoIdForUpdate = $allRepos.find(r => r.github_name === repoFullName)?.id;
      let failures = 0;
      for (const name of toUpdate) {
        try {
          const encrypted = await encryptSecret(key, secretValues[name].trim());
          await createOrUpdateRepoSecret($pat, owner, repo, name, encrypted, key_id);
          // v0.20.0: record secret event for timeline
          if (repoIdForUpdate !== undefined) {
            await recordSecretEvent(repoIdForUpdate, 'set', name).catch((e) => console.warn('record_secret_event failed:', e));
          }
        } catch {
          failures++;
        }
      ...
      if (failures === 0) {
        addToast($tStore('secrets.updatedCount' as any).replace('{0}', String(toUpdate.length)), 'success');
      } else {
        addToast($tStore('secrets.pushPartialFail' as any).replace('{0}', String(failures)).replace('{1}', String(toUpdate.length)), 'error');
      }
    } catch (err: any) {
      if (err?.status === 403 || err?.status === 401) {
        addToast($tStore('secrets.permissionError' as any), 'error');
          ...
          {:else}
            ...
            {#if hasSelectedSecrets}
              <div class="bulk-actions">
                {#if selectedWithValues.length > 0}
                  <button
                    class="bulk-btn update"
                    onclick={() => (showUpdateConfirm = true)}
                    disabled={pushing}
                    type="button"
                  >
                    {$tStore('secrets.updateSelected' as any)} ({selectedWithValues.length})
                  </button>
                {/if}
                <button
...
<!-- Update selected confirmation -->
{#if showUpdateConfirm}
  <ConfirmDialog
    title={$tStore('secrets.updateSelectedConfirm' as any).replace('{0}', String(selectedWithValues.length))}
    message={selectedWithValues.join(', ')}
...
<style>
  ...
  .bulk-btn.update:hover:not(:disabled) {
