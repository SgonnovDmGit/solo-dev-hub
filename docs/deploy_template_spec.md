...
| `@@DEPLOY_BRANCH@@` | string | `master` | Ветка-триггер push'а |
| `@@NETWORK_NAME@@` | string | `swan_prod_proxy-network` | Docker network контейнера |
| `@@CONTAINER_NAME@@` | string | `swan-support-prod-frontend` | `--name` контейнера + NPM `forward_host` (docker DNS резолвит имя внутри proxy-network). v0.25.0+: placeholder, не secret. Per-env уникален |
| `@@COMPOSE_PROJECT@@` | string | `swan_prod` | label `com.docker.compose.project` |
| `@@COMPOSE_SERVICE@@` | string | `swan-support-prod-frontend` | label `com.docker.compose.service` (обычно совпадает с CONTAINER_NAME) |
