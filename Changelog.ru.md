...
Документация для контрибьюторов: обзор архитектуры, генерируемый справочник схемы SQLite и проход по публичным докам, которые разъехались с реальностью.

### Added
- **`docs/ARCHITECTURE.md` (T-000082)** — карта для человека-контрибьютора: схема слоёв, зона ответственности каждого Rust- и фронтенд-модуля, несущие решения *с обоснованием* (Tauri вместо Electron, SQLite вместо JSON, асимметричный синк Markdown↔БД, копирование файлов вместо git-remote, шифрование секретов at-rest, git через системный CLI), кросс-репо flow требований sequence-диаграммой и индекс всех флоу-доков. `CLAUDE.md` покрывает то же для AI — этот документ для людей.
- **`docs/schema/` — генерируемый справочник SQLite (T-000083)** — ER-диаграмма плюс страница на каждую таблицу (18 таблиц, VIEW нет, схема v28), собирается [tbls](https://github.com/k1LoW/tbls) через `npm run schema:docs`. Скрипт поднимает временную БД прогоном всех миграций по пустому файлу — доки описывают код, а не чью-то рабочую базу; повторный прогон даёт побайтово тот же результат. Описания таблиц и колонок живут в `.tbls.yml` (в SQLite нет `COMMENT ON`) и подставляются в сгенерированные страницы.

### Changed
- **`CLAUDE.md` пересобран под реальность (T-000158)** — AI-карта проекта разъехалась настолько, что вводила в заблуждение: описывала схему v22 (фактически v28), числила живым `bug_stats` VIEW, снесённый ещё в v23, называла `project_microservices` связкой проект↔репозиторий (с v12 это проект↔проект), утверждала что `DeployScreen` удалён (он жив как тело таба Deploy), а лог эволюции обрывался на v0.27.0. Заодно обновлена карта модулей после сплитов v1.4.x/v1.5.0 и базовая линия тестов.
- **Публичные доки приведены в соответствие (T-000159)** — оба README заявляли бинарь *~11 MB* (реально ~19 MB) и *~790* i18n-ключей (реально ~850), а их Roadmap-секции всё ещё показывали **v1.4.1 как текущую** с номерами будущих версий, которые реальность давно обогнала. Переписано по фактической истории релизов, `docs/roadmap.md` перепланирован вокруг трёх оставшихся вех. В обоих README появился раздел для контрибьюторов со ссылками на новые доки.
- **Правило закрытия релиза исправлено в источнике (T-000159)** — скилл `sdh-release-closure` ограничивал шаг регенерации схемы *«только серверными проектами с БД»* и явно исключал десктоп/CLI. Именно эта формулировка объясняет, почему в этом репо — десктопе с 28 миграциями — схема ни разу не регенерировалась. Теперь триггер — наличие БД, а не тип проекта.

### Tests
- 459 Rust (1 ignored: генератор схемы), 86 vitest, svelte-check чист.

## [1.9.6] — 2026-07-19

Догфуд-патч: дублированный id задачи в `todo.md` больше не срывает синк задач репозитория.

### Fixed
- **Дубль id задачи в одном MD-файле больше не ломает портфельное обновление (T-000157).** Одна и та же задача под двумя хедерами `## vX.Y.Z` — типичный след перепланирования, когда старую строку не удалили, — заставляла синк вставлять id дважды и ловить `UNIQUE(repository_id, task_id, source)`. Синк репо обрывался до простановки `tasks_migrated_at`, поэтому ↻ на Дашборде на каждое нажатие выдавал `Tasks <репо>: UNIQUE constraint failed…`, а задачи этого репо вообще не доезжали до БД. Теперь повторяющиеся id схлопываются до сверки с БД — остаётся **последнее** вхождение (более поздний version-хедер = более свежее намерение), в лог пишется предупреждение; done.md обрабатывается так же.

### Tests
- 459 Rust (+1: синк с дублем id), 86 vitest, svelte-check чист.

## [1.9.5] — 2026-07-07

Догфуд-патч: закрытый кросс-репо REQ больше не воскресает у получателя, плюс уточнения воркфлоу-скиллов (раскладка задач, стадия перепланирования, гигиена todo).

### Fixed
- **Закрытый REQ больше не всплывает как «открытый» у получателя (T-000155).** Закрытие пары = положить `.impl.md` и дать одному синку снести всю тройку. Но если файлы удалили руками (impl/response убрали, а base `REQ-NNN.md` оставили), base отправителя — source of truth, и каждый следующий синк заново копировал его получателю без квитанции — выполненное требование выглядело открытым. Само приложение такое состояние произвести не может (оно удаляет base первым), поэтому фикс поведенческий: REQ-скиллы теперь явно запрещают ручное удаление и описывают failure mode воскрешения. Код-сеть: `scan_requirements` теперь исключает `*.impl.md`, чтобы застрявший impl-файл (если teardown когда-то оборвётся на полпути, напр. Windows-лок) не был принят за требование и не уехал фантомной парой.

### Changed
- **REQ-скиллы прямо говорят «положил `.impl.md` — СТОП» (T-000155).** `sdh-cross-repo-req-send` §5 теперь: `.impl.md` — это всё действие закрытия, не удаляй base / response / impl руками, дай пройти синку. Добавлен `.impl.md` в список запрета ручного удаления + врезка про «воскрешение base-REQ». В `sdh-cross-repo-req-answer` добавлено «не удаляй свой `.response.md` на уборку».
- **Задачи всегда на роадмапе, никакого беклога (T-000156).** В `sdh-release-lifecycle` добавлено правило «Task filing»: каждая задача в `docs/todo.md` кладётся под `## vX.Y.Z`-заголовок; единственная допустимая неопределённость — тайминг относительно следующего MAJOR (кладём до- или после-мажора, но всё равно на таймлайн) — без безымянной кучи. Ссылки из стадии Plan и `sdh-phase-workflow`.
- **Стадия перепланирования переосмыслена как переоценка, не дизайн (T-000156).** Stage 10 теперь — короткая переоценка/перенарезка уже намеченных задач следующего релиза (пересмотр effort/priority, дробление распухших, отсев неактуальных) — явно НЕ спека/брейншторм/«фаза 2». Дёшево, ожидается на каждом закрытии, но ограничено.
- **Закрытие удаляет выполненные задачи из todo.md начисто (T-000156).** Шаг 4 в `sdh-release-closure` теперь запрещает оставлять пометки `done`/`✓`, зачёркнутые строки и прозу «что и как» в todo.md — завершённость живёт только в done.md.
- **Переписан flow-док requirements-sync.** `docs/flows/requirements-sync.md` застрял на v0.14.0 (монолитные `sync.rs`/`lib.rs`, только ручной ✓). Теперь описывает распиленные модули, `.impl.md` авто-close, авто-синк/авто-коммит v1.7.0 и failure mode воскрешения.

### Tests
- 458 Rust (`cargo test --lib`; +1 guard на исключение impl из scan_requirements) / 86 vitest / 0 ошибок svelte-check. Без миграции БД. Изменения скиллов/доков — текст, проверено ревью.

## [1.9.4] — 2026-07-04

Догфуд-патч: фоновый авто-синк больше не морозит окно.

### Fixed
- **Авто-синк больше не морозит UI (T-000154).** `sync_project` была синхронной Tauri-командой, поэтому её тяжёлая работа — запись skeleton-файлов в каждый репо плюс портфельный авто-коммит, спавнящий множество `git`-субпроцессов — шла на главном (UI) потоке и замораживала окно WebView2 на весь прогон. Фоновый таймер авто-синка запускал её периодически — это и выглядело как короткие подвисания каждые несколько секунд. Теперь команда исполняется вне главного потока (`#[tauri::command(async)]`), и окно остаётся отзывчивым во время синка; ручной Sync тоже выигрывает. Тело не тронуто (`AppDb` — `Send + Sync`, лок БД берётся по-методно, не держится на весь прогон). Заморозка стала заметна сразу после v1.9.3: тот релиз починил краш авто-коммита, и авто-коммит стал делать полную git-работу вместо раннего обрыва — этого хватило, чтобы блокировка главного потока стала ощутимой.

### Tests
- 457 Rust / 86 vitest / 0 svelte-check. Вынос с главного потока — рантайм-поведение, юнит-тестом не проверяется; верифицировано смоуком (авто-синк включён с коротким интервалом, окно отзывчиво во время синка). Новых тестов нет, миграций БД нет.

## [1.9.3] — 2026-07-04

Догфуд-патч: авто-коммит больше не падает на гитигнорируемых кросс-репо папках, плюс два уточнения правил/скиллов для более чистого поведения нейронок.

### Fixed
- **Авто-коммит пропускает гитигнорируемые кросс-репо папки вместо падения (T-000152).** Когда репо-получатель держит входящий кросс-репо инбокс (`docs/*-requirements/`, `docs/*-announcements/`) в `.gitignore`, `git add` отказывался добавлять игнорируемые пути и ронял весь авто-коммит синка (`git add exit 1: paths are ignored`). Теперь авто-коммит определяет гитигнорируемые папки (`git check-ignore`) и пропускает их — коммитит только реально отслеживаемые кросс-репо папки (напр. синкнутый API-контракт), а «все папки игнорируются» трактует как чистый no-op. Гитигнорируемые папки — намеренный локальный инбокс, их никогда не форсят через `-f`.

### Changed
- **Правило авто-коммита уточняет, что gitignore — равноправная альтернатива (T-000153).** Always-on правила теперь говорят, что коммитить синкнутые файлы и держать их в `.gitignore` — два равноправных способа не читать их как «foreign»: авто-коммит уважает разделение (коммитит отслеживаемые папки, игнорируемые не трогает), а два механизма дополняют друг друга, а не конфликтуют.
- **Гейт визуальной приёмки в релиз-жизненном цикле (T-000153).** Для client / landing / любого визуального релиза смок стадии Test = пользователь смотрит собранный/запущенный UI: closure и гит-блок тега/мерджа не показываются, пока пользователь явно не одобрил визуал — даже в автономном режиме — и блок выдаётся один раз, а не переспамливается каждый ход. Сервер / тул / микросервис верифицируются автосмоком (без визуального гейта). Устраняет преждевременные гит-блоки и повторные прогоны команд.

### Tests
- 457 Rust (`cargo test --lib`; +3 guard'а на пропуск gitignore), 86 vitest, 0 ошибок svelte-check. Без миграции БД.
