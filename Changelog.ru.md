...
Docs/rules патч: скиллы API-контракта теперь требуют партиционировать контракт по аудитории-потребителю и по объёму вместо одного монолитного `api.md`. Поведение приложения не меняется.

### Changed
- **API-контракт партиционируется по аудитории + объёму (T-000151).** Скилл `sdh-api-contract-maintain` теперь делает разбиение контракта обязанностью, а не опцией: один файл на аудиторию-потребителя в `docs/my_api/` (`client-api.md` / `admin-api.md` / `public-api.md` / `<name>-api.md`, не смешивать) плюс жёсткий триггер по объёму — дели файл после ~12 эндпоинтов / ~400 строк в `<audience>-api-<topic>.md`. После сплита старый монолитный `api.md` должен быть удалён (нет дрейфа, нет двойной доставки). Плоский синк-ко-всем задокументирован как намеренный (даёт препрод-диагностику со стороны клиента). Скилл `sdh-api-contract-consume` соответственно грепает весь набор `docs/server-api/*.md`. Чинит скатывание агентов к одному `api.md`.

### Tests
- 454 Rust / 86 vitest / 0 svelte-check (только текст skill-шаблонов; без изменений кода/схемы).

## [1.9.1] — 2026-07-03

Догфуд-патч: два бага из повседневного использования плюс чистка синка скиллов и доработка release-скилла. Error/warning-тост теперь тянется на всю ширину снизу, чтобы длинные сообщения читались; поле авто-коммита в шапке репо стало настоящим выбором ветки; устаревшие on-demand скиллы вычищаются при синке правил; практика «смок на препроде» прописана в release-скилл.

### Fixed
- **Error/warning-тост на всю ширину снизу (B-000027).** Персистентные error/warning-тосты тянутся на всю ширину снизу, а не переносятся в узкой колонке 460px — длинному тексту есть горизонтальное место (скролл + копирование + закрытие без изменений). Транзиентные success/info остаются компактными справа снизу.
- **Ветка авто-коммита — настоящий выбор ветки (B-000028).** Поле авто-коммита в шапке репо получило datalist веток из GitHub (как `DEPLOY_BRANCH` в деплое) вместо слепого текстового ввода; без PAT / офлайн деградирует в обычный текст. Пустые ветки авто-коммита теперь по умолчанию `dev`, если она есть в репо — проставляется по всему портфелю сразу после загрузки репозиториев (идемпотентно, самоограничиваясь), а не только при заходе на экран репо.
- **Устаревшие on-demand скиллы вычищаются при синке правил (T-000149).** «Sync AI rules» теперь удаляет `sdh-*` папки скиллов (и копии `docs/sdh_skills/*.md`), которых больше нет в бандле — объединённые до-сплитовые скиллы перестают висеть после разбивки 6→12. Трогается только namespace `sdh-` — пользовательские скиллы не затрагиваются.

### Changed
- **Практика «смок на препроде» задокументирована (T-000150).** Стадия Test в `sdh-release-lifecycle` теперь явно описывает, что `.env.preprod` хранит креды для *входа* в препрод под смок (ключ к админке, ключи проекта/юзера; OTP: смок-email + кэш refresh-токена) — доступ для смока, а не набор секретов для *запуска* препрода — чтобы прекратить регулярную путаницу.

### Tests
- 454 Rust (`cargo test --lib`; +2 стража прунинга скиллов), 86 vitest, 0 ошибок svelte-check. Без миграции БД.

## [1.9.0] — 2026-07-03

Глобальные правила AI → on-demand скиллы, разбитые по ролям и доработанные. Воркфлоу-тяжёлые правила уезжают из always-on блока CLAUDE.md (~750 → ~215 строк) в **двенадцать** сфокусированных on-demand скиллов. Не просто перенос: каждый кластер разбит по реальной границе ролей (отправитель vs получатель, производитель vs потребитель), а сам release-воркфлоу переработан — каждое правило доступно ровно когда релевантно и всплывает только той стороне, что реально делает эту работу.

### Added
- **Воркфлоу-правила как двенадцать on-demand скиллов (T-000138 / T-000143 / T-000144 / T-000145).** «Sync AI rules» ставит `sdh-*` скиллы в `~/.claude/skills/` (Claude Code всплывает каждый по `description` только когда релевантно) и зеркалит тот же контент в `docs/sdh_skills/*.md` каждого репо для любого AI-агента. Один бандл-источник → обе цели, рассинхрона нет. Кросс-репо кластеры разбиты по роли, чтобы клиент / сервер / микросервис грузил только свою сторону: REQ → `req-send` / `req-answer`; анонсы → `announce-send` / `announce-read`; api.md-контракт → `api-contract-maintain` / `api-contract-consume`. Release-кластер разбит по триггеру: `release-lifecycle` (стадии) / `release-closure` (чеклист + git-блок) / `retro`.
- **Новый скилл `sdh-visual-mockups`.** Выделенный дизайн-гейт для не-серверной визуалки (экран / лендинг / компонент): «показать, а не описать» любым доступным агенту preview (артефакты / HTML / рендеры), варианты рядом, pixel-precise итерации; принятый мокап несёт дизайн-решения.
- **Release-воркфлоу переработан (вшит в скиллы).** Лёгкий vs полный трек (мелкие релизы by default сворачивают План / Ревью-плана); оверлей автономного режима (один front-loaded гейт на принятии дизайна, дальше Внедрение → Тест → документная часть Закрытия без пер-стадийных апрувов; git-блок остаётся user-only); класс бампа решается на Анализе, не на закрытии; late-catch правило (мелкий уверенный фикс, найденный на закрытии, сворачивается в текущий релиз); канонический безопасный для терминала PowerShell git-блок (guard чистого дерева, `$MAIN` master/main, `switch`, аннотированный тег) — команды закрытия перестают импровизироваться; шаг build-артефакта на закрытии («никогда не тегай релиз, чья сборка не показана рабочей»).
- **Trigger-индекс + указатель для не-Claude (T-000145 / T-000146).** Тонкое ядро сохраняет контракты форматов / версионирования / коммитов и компактный индекс «когда ты… → скилл» (12 строк). Секция `CLAUDE.md` каждого репо направляет не-Claude агентов в `docs/sdh_skills/`; папка gitignored (локальная, app-синк).

### Changed
- **Always-on правила ужаты (~750 → ~215 строк).** Воркфлоу-кластеры больше не грузятся в каждую сессию — только on-demand. «Sync AI rules» сидит скиллы рядом с тонким ядром (T-000147); README описывает новую доставку (T-000148). Проектный git-flow док переходит на `switch` + канонический блок закрытия для паритета со скиллом.

### Tests
- 452 Rust (`cargo test --lib`; real-bundle страж теперь проверяет, что все двенадцать скиллов сидятся с валидным frontmatter и рендерятся в `docs/sdh_skills/`), ~86 vitest, 0 ошибок svelte-check. Без миграции БД.

## [1.8.0] — 2026-07-02

Раздел «Отчёты» + экспорт deploy-отчёта в CSV + аудит пушей секретов. Deploy-отчёт переезжает под новый раздел 📋 «Отчёты» (место под будущие отчёты без разрастания титлбара), получает экспорт в CSV, и к нему добавляется «Аудит секретов», который сверяет то, что SDH залогировал при пуше, с тем, что реально лежит в секретах GitHub сейчас.

### Добавлено
- **Раздел «Отчёты» (T-000142).** Отдельная кнопка deploy-отчёта в титлбаре заменена на 📋 «Отчёты» — хаб с карточками-drill-in (Деплои и Аудит секретов), чтобы будущие отчёты не плодили кнопки в титлбаре (осталось 6).
- **Экспорт deploy-отчёта в CSV (T-000140).** Кнопка 📥 «Сохранить CSV» на deploy-отчёте выгружает текущие отфильтрованные строки (WYSIWYG: репо, окружение, домен, ветка, образ, имя БД, число секретов, обновлён) в выбранный пользователем файл через системный save-диалог. Разделитель `;` (Excel RU/EU открывает по колонкам сразу), RFC4180-экранирование, стабильные английские заголовки, без новых зависимостей; чувствительные значения скрыты ровно как в UI. Фронт уплощает отображаемые строки, backend только форматирует + пишет (единый источник display-логики, экранирование покрыто cargo-тестами).
- **Аудит секретов (T-000135).** Новый отчёт «Аудит секретов»: портфельный журнал залогированных пушей секретов (имя секрета + цель репо/окружение + set/delete + время — никогда значения) плюс per-repo сверка того, что SDH залогировал при пуше, с секретами, реально присутствующими в GitHub сейчас (на уровне репо и по каждому deploy-окружению), разложенная на корзины: в-обоих / в-журнале-нет-в-GitHub / в-GitHub-нет-в-журнале. Сверка требует GitHub-токен; журнал работает и без него. Переиспользует существующий лог sync/deploy-событий — без новой таблицы и миграции.

### Тесты
- 446 cargo / 86 vitest / 0 svelte issues. Без миграции схемы (аудит читает существующие таблицы `sync_events`/`deploy_events`).

## [1.7.0] — 2026-07-02

Автоматизация воркфлоу кросс-репо синхронизации (боли №2b/2c/3). Синк теперь умеет работать по фоновому таймеру, автокоммитить файлы, которые копирует в репо-получатели (чтобы локальный AI не считал их «чужими»), и авто-закрывать REQ-пары, подтверждённые отправителем — плюс папка для многофайловых API-контрактов и фикс копируемого тоста ошибок. Всё app-side; шаблон глобальных правил документирует три новых контракта.

### Added
- **Фоновый авто-синк по таймеру (T-000136).** Per-project тумблер «Авто-синк» (в шапке проекта) + глобальный мастер-переключатель и интервал (5–120 мин, дефолт 15) в Настройках гоняют синк для включённых проектов с фиксированной проверкой раз в 60с, с гейтом на истечение интервала и дебаунсом после ручного синка. Время последнего авто-синка показано в Настройках. Работает только пока окно приложения открыто.
- **Автокоммит синкнутых кросс-репо файлов (T-000137).** Per-repo поле «Автокоммит» с веткой (пусто = выкл) в шапке репозитория. После синка репо с заданной веткой получают маркер-коммит — автор `Solo Dev Hub <noreply@solodevhub.app>`, префикс `chore(sdh-sync):` — только изменений в кросс-репо папках, pathspec-scoped (прочая работа в дереве не тронута) и только если репо на заданной ветке (иначе пропуск + репорт). Чинит ситуацию, когда локальный AI получателя считает синкнутые файлы «чужими» незакоммиченными.
- **Авто-закрытие REQ через `.impl.md` (F-000039).** Отправитель кладёт (возможно пустой) `REQ-NNN_<slug>.impl.md` в свою исходящую папку как сигнал «интегрировал»; следующий синк сносит всю тройку (`.md`/`.response.md`/`.impl.md`) с обеих сторон автоматически. Бейдж «⏳ impl» помечает пары к авто-закрытию в экране синка, прогон репортит сколько пар закрыто; ручная кнопка ✓ остаётся для репо, не перешедших на файл.
- **Многофайловые API-контракты через `docs/my_api/` (T-000139).** Отправитель может разбить контракт на несколько markdown-файлов в папке `docs/my_api/`; синк слепо копирует все `*.md` в существующий инбокс получателя (`docs/server-api/` или `docs/microservice-api/<name>/`). Старое копирование одиночных `docs/api.md` / `docs/handlers.md` продолжает работать (backward-compatible).

### Fixed
- **Тосты ошибок и предупреждений читаемы и копируемы (B-000027).** Больше не исчезают через 5 секунд, рендерятся как выделяемая скроллируемая панель (max-height 40vh) с кнопкой копирования и явным ×, контейнер шире (460px). success/info тосты сохраняют 5-секундный click-to-dismiss.

### Tests
- 433 cargo / 86 vitest / 0 svelte. Миграция схемы v28 (`repositories.autocommit_branch`, `projects.auto_sync_enabled`).

## [1.6.0] — 2026-07-01

Персист значений деплоя (боль №1). Выбранные значения секретов деплоя теперь можно хранить локально (шифровано), чтобы они предзаполнялись, а не перевбивались каждую сессию; в deploy-отчёте появилась строка с именем базы per-окружение. Изначально планировалось как отдельный типизированный secrets-vault — при проектировании переосмыслено в deploy-интегрированную фичу (БД относится к деплою).

### Added
- **Опциональный локальный персист значений секретов деплоя.** Тумблер 💾 «сохранять» per-секрет (перед колонкой «Вкл.») в таблице секретов деплоя кладёт значение в новое шифрованное хранилище (`deploy_secret_values`, AES-256-GCM, ключ в OS keyring — переиспользует крипту наборов секретов, без мастер-пароля). Сохранённые значения предзаполняют textarea при загрузке вместо пустого поля; выключение тумблера удаляет запись. Доступно только для included + override секретов. Миграция v27.
- **Имя базы в deploy-отчёте.** Под строкой окружения — sub-row с именем базы из сохранённых значений и плейнтекст-плейсхолдеров, с маркером 💾 (хранится локально) / ☁ (только в GitHub). Чувствительные значения (пароли, ключи, токены) не показываются; `DATABASE_URL` редактится до host/db. Host/user и SSH-поля собираются запросом, но пока скрыты в UI (под будущие фильтры).

### Tests
- 413 cargo / 86 vitest / 0 svelte.
