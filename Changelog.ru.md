...
Глобальные правила AI → on-demand скиллы. Воркфлоу-тяжёлые правила (кросс-репо требования, анонсы, api.md-контракт, phase-воркфлоу, release-lifecycle, flow-доки) уезжают из always-on блока CLAUDE.md в on-demand скиллы — always-on контекст ужимается с ~750 до ~215 строк, при этом каждое правило доступно ровно когда релевантно.

### Added
- **Воркфлоу-правила как on-demand скиллы (T-000138 / T-000143 / T-000144 / T-000145).** «Sync AI rules» теперь ставит шесть `sdh-*` скиллов в `~/.claude/skills/` — Claude Code всплывает каждый по его `description` только когда релевантно — и зеркалит тот же контент в `docs/sdh_skills/*.md` каждого репо, чтобы его читал любой AI-агент, не только Claude Code. Один бандл-источник рендерится в обе цели, рассинхрона нет.
- **Trigger-индекс в тонком ядре (T-000145).** Always-on ядро CLAUDE.md сохраняет мелкие контракты форматов / версионирования / коммитов и получает компактный индекс «когда ты… → скилл».
- **Указатель для не-Claude + gitignore (T-000146).** Секция `CLAUDE.md` каждого репо направляет не-Claude агентов в `docs/sdh_skills/`; папка gitignored (локальная, app-синк) как и остальной операционный слой.

### Changed
- **Always-on правила ужаты (~750 → ~215 строк).** Кластеры кросс-репо REQ / анонсов / api.md-контракта / phase-воркфлоу / release-lifecycle / flow-доков больше не грузятся в каждую сессию — только on-demand. «Sync AI rules» в Settings теперь сидит скиллы рядом с тонким ядром (T-000147); README описывает новую доставку (T-000148).

### Tests
- 452 Rust (`cargo test --lib`; +1 real-bundle страж — все шесть скиллов сидятся с валидным frontmatter и рендерятся в `docs/sdh_skills/`), ~86 vitest, 0 ошибок svelte-check. Без миграции БД.
